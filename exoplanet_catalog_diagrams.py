...
rv_mask = np.where((rv_dist >= -np.inf)&(rv_dist <= np.inf))
...
nott = pd.read_csv(path+'CA_NGPPS_MODEL_contrast.csv')
nott_sma = nott['pl_r_uni_detected'].values[::10]
nott_contrast = nott['pl_contrast_uni_detected'].values[::10]
...
fig = plt.figure(figsize=(width, height))
ax = plt.subplot(111)
# ax.scatter(list(transit_sma)+list(rv_sma), list(transit_contrast)+list(rv_contrast), label='Indirect detection', marker='*')
ax.scatter(transit_sma, transit_contrast, label='Transit', marker='*')
ax.scatter(rv_sma, rv_contrast, label='Velocité radiale', s=75)
ax.scatter(imaging_sma, imaging_contrast, label='Imagerie directe', marker='^', s=75)
ax.scatter(nott_sma, nott_contrast, label='Population synthétique', marker='x', s=75, alpha=0.5, c='gray')
ax.axvline(3.8, ls='--', c=colours[5])
ax.text(3.9, 1e-2, "Ligne de glace", multialignment='left', fontsize=text_sz, c=colours[5], rotation=-90)
ax.axhline(1e-4, ls=':', c=colours[3])
ax.text(6, 5e-5, "ELT/ANDES", multialignment='left', fontsize=text_sz, c=colours[3], rotation=0)
ax.axhline(1e-5, ls=':', c=colours[3])
ax.text(6, 5e-6, "ELT/MOSAIC", multialignment='left', fontsize=text_sz, c=colours[3], rotation=0)
# ax.axvline(1., ls='-.', c=colours[6])
# ax.text(0.6, 0.2, "VLTI", multialignment='left', fontsize=text_sz, c=colours[6], rotation=-90)
# ax.axvline(3.6, ls='-.', c=colours[6])
# ax.text(2., 0.2, "ELT", multialignment='left', fontsize=text_sz, c=colours[6], rotation=-90)
# ax.text(1.2, 7e-3, "HZ\n (Sun-like Star)", multialignment='center', fontsize=text_sz*0.95, c=colours[4], rotation=-90)
# ax.fill_between((100, xlims[1]), 1e-6, ylim[1], alpha=0.2, facecolor=colours[2])
# ax.text(8e3, 1.2e-6, "ELT/HARMONI", multialignment='right', fontsize=text_sz, c=colours[2], rotation=0)
ax.fill_between((1, xlims[1]), 1e-3, ylim[1], alpha=0.2, facecolor=colours[2])
ax.text(8e3, 1.2e-3, "VLTI/MATISSE", multialignment='right', fontsize=text_sz, c=colours[2], rotation=0)
ax.fill_between((1, xlims[1]), 1.e-5, ylim[1], alpha=0.2, facecolor=colours[0])
ax.text(1.15e4, 1.3e-5, "VLTI/Asgard", multialignment='right', fontsize=text_sz, c=colours[0], rotation=0)
ax.fill_between((1, xlims[1]), 7e-5, ylim[1], alpha=0.2, facecolor=colours[1])
ax.text(0.85e4, 1.5e-4, "VLTI/GRAVITY", multialignment='right', fontsize=text_sz, c=colours[1], rotation=0)
ax.set_xscale('log')
ax.set_yscale('log')
ax.set_ylim(*ylim)
ax.set_xlim(xlims)
...
ax.legend(loc='lower right', fontsize=text_sz, ncol=2)
ax.set_xlabel('Demi-grand axe (AU)', size=20)
ax.set_ylabel('Luminosité relative', size=20)
